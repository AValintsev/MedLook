
namespace Nop.Ithoot.Plugin.Shipping.NovaPoshta
{
    /// <summary>
    /// Represents constants of the "Fixed or by weight" shipping plugin
    /// </summary>
    public static class NovaPoshtaDefaults
    {
        /// <summary>
        /// The key of the settings to save fixed rate of the shipping method
        /// </summary>
        public const string FixedRateSettingsKey = "ShippingRateComputationMethod.NovaPoshta.Rate.ShippingMethodId{0}";

        /// <summary>
        /// The key of the settings to save transit days of the shipping method
        /// </summary>
        public const string TransitDaysSettingsKey = "ShippingRateComputationMethod.NovaPoshta.TransitDays.ShippingMethodId{0}";
    }
}
